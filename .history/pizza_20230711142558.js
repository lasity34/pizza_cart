function cart() {
  return {
    ...
    increment(size) {
      if (size === "small") this.smallAmount++;
      else if (size === "medium") this.mediumAmount++;
      else if (size === "large") this.largeAmount++;
    },
    decrement(size) {
    ...
    paymentTotal() {
      if (this.paymentAmount < this.total()) {
        setTimeout(() => {
          this.message = `R${(this.total() - this.paymentAmount).toFixed(
            2
          )} is needed`;
        }, 3000);
      } else if (this.paymentAmount > this.total()) {
        setTimeout(() => {
          this.message = `There is R${(
